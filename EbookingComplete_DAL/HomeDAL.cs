...
namespace EbookingComplete_DAL
{
    public class HomeDAL
    {
        ...
        public DataSet eBookingMyLogSend(Int64 roid,int isclassified, int userid)
        {
            DataSet ds = new DataSet();
            using (var db = new DBManager())
            {
                ...
                db.AddParameters(0, "@ROID", roid);
                db.AddParameters(1, "@IsClassified", isclassified);
                db.AddParameters(2, "@UserID", userid);
                ds = db.ExecuteDataSet(CommandType.StoredProcedure, "EBK_SP_SENDFORSCREENING");
            }
            return ds;
        }
